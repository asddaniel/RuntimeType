...
export const getDataType = (data:any)=>{
       ...
       if(typeof(data)=='object') return getObjectTypes(data)
...
const isEqual = (object1:any, object2:any) =>{
    ...
    for (const l of first){
        ...
        if(first.charAt(incr)!=two.charAt(incr)) return false
        incr++;
    } 
return true
...
export const Type = function createTypedProxy<T>(
    initialValue: T, 
    errorCallback?: (expectedType: string|any, receivedType: string|any) => void
): T {
